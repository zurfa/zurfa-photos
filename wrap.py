...
def import_directory(Dir=False):
	...
	try:
		...
		try:
			...
			lg.logger.critical("Could not create import session directory, can not continue.")
			return False
		else:
			...
			for Index,File in enumerate(Files,1):
				lg.logger.info("Importing file %s/%s %s" % (Index,NumFiles,File))
...
def import_file(File,Export=False,Options=None):
	...
	try:
		...
		if UFID:
			DEST	= "%s%s%s"% (config.DIR_LIBRARY,UFID,os.path.splitext(File)[1])
				...
				if fi.copy_file(File,DEST):
					...
					if os.path.isdir(Export):
						...
						if fi.move_file(File,Export):
							# Successfully moved original file to processed files
							return Data
						else:
